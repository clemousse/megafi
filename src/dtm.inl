...
template<Primitive prim> inline void DTM::build_line(unsigned long) {}
template<Primitive prim> inline void DTM::build_line(unsigned long) const {}

template<Primitive prim> inline void DTM::build_back(unsigned long) {}
template<Primitive prim> inline void DTM::build_back(unsigned long) const {}
...
TEMPLATE(build_line, POINT, unsigned long i, buildFunction(i);)
...
TEMPLATE(build_line, TRILINE, unsigned long i,
         buildFunction(i);
         if(i < m_vertices.size() - m_lineLength) // not at last line
             buildFunction(i + m_lineLength);)

TEMPLATE(build_back, TRILINE, unsigned long j, buildFunction(j);)

#define ARRAY_SIZE_TRILINE NB_CALL_LINE*2 - m_lineLength + NB_CALL_BACK



TEMPLATE(build_line, QUADLINE, unsigned long i,  build_line<TRILINE>(i);)

TEMPLATE(build_back, QUADLINE, unsigned long j,
...
TEMPLATE(build_line, TRIFILL, unsigned long i,
    buildFunction(i);
    if(i < m_vertices.size() - m_lineLength) // not at last line
        buildFunction(i + m_lineLength);
    else
...
TEMPLATE(build_line, QUADFILL, unsigned long i, build_line<TRIFILL>(i);)
...
TEMPLATE(build_line, LINELOOP, unsigned long i,
...
TEMPLATE(build_line, TRIANGLES, unsigned long i,
